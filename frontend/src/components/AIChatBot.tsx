...
import { MessageCircle, X, Send, Phone, Sparkles, ArrowRight } from 'lucide-react';
import Link from 'next/link';

type ChatMsg = {
  role: 'user' | 'assistant';
  text: string;
  cta?: 'configure' | 'call' | null;
};

const WELCOME: ChatMsg = {
  role: 'assistant',
  text:
    "Namaste! I'm Kalakriti Sakhi ✨  I can help with pricing, mediums, turnaround times, or your order status. What would you like to know?",
};

const QUICK_PROMPTS = [
  'What does a watercolour A3 cost?',
  'Which medium for a 50th anniversary gift?',
  'Status of my order?',
...
export default function AIChatBot() {
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: 'assistant', text: data.reply, cta: data.cta }]);
    } catch (e) {
      setMessages((prev) => [
        ...prev,
        {
      ...
      {open && (
        ...
        >
          ...
          <form
            ...
            className="p-3 border-t border-[#E0D5C8] bg-white flex items-center gap-2"
          >
            <input
